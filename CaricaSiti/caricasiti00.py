...
#        QMessageBox.warning(self.iface.mainWindow(), "Titolo della Finestra","Ciao Ciao", QMessageBox.Ok, QMessageBox.Ok)
    ...
    def Scrivi_dati(self,lym):
       ...
       while itero.nextFeature(fecl):
          ...
          comandoSql = 'SELECT "x_ant","y_ant","h","n_tx","pot_tx","tilt","direzione","idmod","fmin","fase","note1" FROM etere.antenne WHERE "idsito" = ' + strIdSito
          if not self.Cmd_sql(comandoSql):
             return
          tupAnt = self.cur.fetchall()
#          QMessageBox.warning(self.iface.mainWindow(), "Titolo della Finestra",str(len(tupAnt))+str(tupAnt[0]), QMessageBox.Ok, QMessageBox.Ok)
          for j in tupAnt:
             numAnt += 1
             if j[10] == None:
                note1 = "Nota vuota"
             ...
             if not (strIndMod in listaIdMod):
                listaIdMod.append(strIndMod) 
             rigaOut = str(j[0]) + " " + str(j[1]) + " " + str(j[2]) + " " + str(j[3]) + " " + str(j[4]) + " " + str(j[5]) + " "
             rigaOut = rigaOut + str(j[6]) + " " + str(j[7]) + " " + str(j[8]) + " " + str(j[9]) + " " + strIdSito + " " + note1 + "\r\n" 
             fileSorg.write(rigaOut)
          self.aggProgBar()
       self.clearProgBar()
       fileSorg.close()
       fileSorg = open(nomeFileSorg,'r+')
       contenuto = fileSorg.read()
       fileSorg.seek(0)
       fileSorg.write(str(numAnt) + "\r\n")
       fileSorg.write(contenuto)
       fileSorg.close()

       fileAnt = open(nomeFileAnt,'w')
       self.newProgBar("Scarico dal Database Etere i diagrammi di antenne",len(listaIdMod))
       for j in listaIdMod:
          ...
          fileAnt.write(j+'\r\n')
          fileAnt.write(str(tupMod[0][2])+'\r\n')
          fileAnt.write(str(tupMod[0][3])+'\r\n')
          for k in tupDia:
             fileAnt.write(str(k[0]) + ' ' + str(k[1]) + '\r\n')
          for k in tupDia:
             fileAnt.write(str(k[0]) + ' ' + str(k[2]) + '\r\n')
          self.aggProgBar()
       self.clearProgBar()
       fileAnt.close() 
    ...
    def ricarica_siti(self,jLay,x,y,Distanza):
       self.newProgBar("Scarico dal Database Etere siti nel raggio di " + Distanza + " metri",0)
       ...
       laRiga = "ST_Distance(geom,ST_GeomFromText('POINT(" + str(x) + " " + str(y) + ")',3003)) < " + Distanza
